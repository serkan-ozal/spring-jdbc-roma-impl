...
public abstract class ProxyListLoader<T> implements LazyLoader {
	...
	private static final Map<String, Class<ProxyListLoader<?>>> proxyListLoaderClassMap = 
			new HashMap<String, Class<ProxyListLoader<?>>>();
	...
	public static ProxyListLoader createProxyListLoader(String loadingCode, String classPath, Object[] parameters) {
		return createProxyListLoader(null, loadingCode, classPath, parameters);
	}
	
	@SuppressWarnings({ "rawtypes", "unchecked" })
	public static ProxyListLoader createProxyListLoader(String id, String loadingCode, String classPath, Object[] parameters) {
		Class<ProxyListLoader<?>> proxyListLoaderClass = null;
		if (id != null) {
			proxyListLoaderClass = proxyListLoaderClassMap.get(id);
		}
		try {
			if (proxyListLoaderClass == null) {
				...
				proxyListLoaderClass = generatedCls.toClass();
				
				if (id != null) {
					proxyListLoaderClassMap.put(id, proxyListLoaderClass);
				}
			}	
			
			ProxyListLoader pll = (ProxyListLoader)proxyListLoaderClass.newInstance();

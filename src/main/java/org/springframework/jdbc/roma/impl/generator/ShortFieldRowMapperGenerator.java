 ...
 * @author Serkan ÖZAL
 */
public class ShortFieldRowMapperGenerator<T> extends AbstractRowMapperFieldGenerator<T> {

	public ShortFieldRowMapperGenerator(Field field, ConfigManager configManager) {
	...
	public String doFieldMapping(Field f) {
		...
		String setValueExpr = RESULT_SET_ARGUMENT + ".getShort(\"" + columnName + "\")";
		if (paramType.equals(Short.class)) {
			setValueExpr = "Short.valueOf" + "(" + setValueExpr + ")";
		}
		return 
			wrapWithNullCheck(	

 ...
 * @author Serkan ÖZAL
 */
public abstract class ProxySetLoader<T> implements LazyLoader {

	private static final Map<String, Class<ProxySetLoader<?>>> proxySetLoaderClassMap = 
			new HashMap<String, Class<ProxySetLoader<?>>>();
	...
	public static ProxySetLoader createProxySetLoader(String loadingCode, String classPath, Object[] parameters) {
		return createProxySetLoader(null, loadingCode, classPath, parameters);
	}
	
	@SuppressWarnings({ "rawtypes", "unchecked" })
	public static ProxySetLoader createProxySetLoader(String id, String loadingCode, String classPath, Object[] parameters) {
		Class<ProxySetLoader<?>> proxySetLoaderClass = null;
		if (id != null) {
			proxySetLoaderClass = proxySetLoaderClassMap.get(id);
		}
		try {
			if (proxySetLoaderClass == null) {
				ClassPool cp = ClassPool.getDefault();
				CtClass generatedCls = cp.makeClass("ProxySetLoader" + RowMapperUtil.generateRandomClassPostFix());
				generatedCls.defrost();
				generatedCls.setSuperclass((cp.get(ProxySetLoader.class.getName())));
				...
				proxySetLoaderClass = generatedCls.toClass();
				
				if (id != null) {
					proxySetLoaderClassMap.put(id, proxySetLoaderClass);
				}
			}	
			
			ProxySetLoader pll = (ProxySetLoader)proxySetLoaderClass.newInstance();
